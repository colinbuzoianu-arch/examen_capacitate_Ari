...
import { simulareResultEmailHtml } from "../utils/emailTemplates.js";
...
export default function SimularePage({ onBack }) {
  ...
  async function submitSimulare() {
    ...
    try {
      ...
      // Email result to Colin + Ana — fire-and-forget, never block the UI
      try {
        const durataMin = Math.floor((Date.now() - startTs) / 60000);
        const html = simulareResultEmailHtml({
          studentName: user?.name || "Ari",
          subject,
          nota: res.nota,
          totalPuncte: res.totalPuncte,
          subiectI:   { punctaj: res.subiectI.punctaj,   maxim: res.subiectI.maxim },
          subiectII:  { punctaj: res.subiectII.punctaj,  maxim: res.subiectII.maxim },
          subiectIII: res.subiectIII ? { punctaj: res.subiectIII.punctaj, maxim: res.subiectIII.maxim } : null,
          feedbackGeneral:    res.feedbackGeneral,
          capitoleDeRevazut:  res.capitoleDeRevazut || [],
          durataMinute: durataMin,
          appUrl: window.location.origin,
        });
        sendEmail({
          to: ["colinbuzoianu@gmail.com", "anamunteanucontact@gmail.com"],
          subject: `🎓 ${user?.name || "Ari"} a terminat simularea ${subject === "romana" ? "Română" : "Matematică"} · Nota ${res.nota.toFixed(2)}`,
          html,
        });
